...
(*======================================================================
  ...
  ......................................................................*)
(*
Pretty assert. Uses OCaml assertion instead of unit test and adds some
nice styling and a message (inspired by Absbook.unit_test).   
*)
...
module ListQueue (Elt : COMPARABLE) : PRIOQUEUE with type elt = Elt.t = struct
  exception QueueEmpty
  ...
  let test_empty () =
    passert ~suite:"empty" (List.length empty = 0) "Create empty"

  let test_add () =
    let test = passert ~suite:"add" in
    let e = Elt.generate () in
    let a = Elt.generate_gt e in
    let g = Elt.generate_lt e in
    let r = g in
    let t = Elt.generate_gt a in
    test (add e empty = [ e ]) "Single el";
    test
      (empty |> add e |> add a |> add g |> add r |> add t = [ g; r; e; a; t ])
      "Lt, eq, and gt"

  let test_is_empty () =
    let test = passert ~suite:"is_empty" in
    test (is_empty empty = true) "Empty queue";
    test (is_empty (add (Elt.generate ()) empty) = false) "Not empty"
  (* Test after queue add!! *)

  let test_take () =
    let test = passert ~suite:"take" in
    let b = Elt.generate () in
    let a = Elt.generate_lt b in
    let d = Elt.generate_gt b in
    let c = b in
    let q = empty |> add b |> add a |> add d |> add c in
    test
      (try
         let _ = take empty in
         false
...
module TreeQueue (Elt : COMPARABLE) : PRIOQUEUE with type elt = Elt.t = struct
  ...
  module T : ORDERED_COLLECTION with type elt = Elt.t = BinSTree (Elt)
  ...
  let take (q : queue) : elt * queue =
    try
      ...
      (min, T.delete min q)
      (* This would be meaningfully better if we could access pull_min
         from here. *)
    with _ -> raise QueueEmpty

  let to_string (q : queue) : string = T.to_string q

  let test_is_empty () =
    let test = passert ~suite:"is_empty" in
    test (is_empty empty = true) "empty tree"
  (* Test more after add is complete *)
  ...
  let test_add () =
    ...
    let r = g in
    let t = Elt.generate_gt a in
    let q = empty |> add e |> add a |> add g |> add r |> add t in
    test (T.getmax (add e empty) = e && T.getmin (add e empty) = e) "Single el";
    test (T.getmin q = g && T.getmax q = t) "Lt, eq, and gt"
  ...
  let test_take () =
    ...
    let c = b in
    let q = empty |> add b |> add a |> add d |> add c in
    let first, took1 = take q in
    let second, took2 = take took1 in
    let third, took3 = take took2 in
    test
      (try
         let _ = take empty in
         false
      ...
      "Empty queue";
    test
      (first = a && T.getmin took1 = b && T.getmax took1 = d)
      "distinct priority";
    test
      (second = c && T.getmin took2 = b && T.getmax took2 = d)
      "choose oldest";
    test
      (third = b && T.getmin took3 = d && T.getmax took3 = d)
      "verify previous"

  let test_to_string () =
    passert ~suite:"to_string" (to_string empty = "Leaf") "Empty"
...
module BinaryHeap (Elt : COMPARABLE) : PRIOQUEUE with type elt = Elt.t = struct
  exception QueueEmpty
  ...
  *)
  ...
  let to_string (q : queue) =
    let rec to_string' (t : tree) =
      ...
      | OneBranch (e1, e2) ->
          "OneBranch (" ^ Elt.to_string e1 ^ ", " ^ Elt.to_string e2 ^ ")"
      | TwoBranch (Odd, e1, t1, t2) ->
          "TwoBranch (Odd, " ^ Elt.to_string e1 ^ ", " ^ to_string' t1 ^ ", "
          ^ to_string' t2 ^ ")"
      | TwoBranch (Even, e1, t1, t2) ->
          "TwoBranch (Even, " ^ Elt.to_string e1 ^ ", " ^ to_string' t1 ^ ", "
          ^ to_string' t2 ^ ")"
    in
    match q with Empty -> "Empty" | Tree t -> to_string' t

  (* is_empty q -- Predicate returns `true` if and only if `q` is
     an empty queue *)
  let is_empty : queue -> bool = ( = ) Empty
  ...
  let add (e : elt) (q : queue) : queue =
    ...
    (* If the queue is empty, then `e` is the only Leaf in the tree.
       ...
       tree *)
    match q with Empty -> Tree (Leaf e) | Tree t -> Tree (add_to_tree e t)
  ...
  let get_top (t : tree) : elt =
    match t with Leaf e | OneBranch (e, _) | TwoBranch (_, e, _, _) -> e
  ...
  (* This changes the value at the top of a tree. It could easily break the
     invariants
        and should be used *very carefully* *)
  let __replace_top (t : tree) (el : elt) : tree =
    match t with
    | Leaf _ -> Leaf el
    | OneBranch (_, child) -> OneBranch (el, child)
    | TwoBranch (order, _parent, left, right) ->
        TwoBranch (order, el, left, right)

  let rec fix (t : tree) : tree =
    match t with
    | Leaf _ -> t
    | OneBranch (parent, child) -> (
        match Elt.compare parent child with
        | Less | Equal -> t
        | Greater -> OneBranch (child, parent))
    | TwoBranch (balance, el, left, right) -> (
        let left_el = get_top left and right_el = get_top right in
        match
          ( Elt.compare el left_el,
            Elt.compare el right_el,
            Elt.compare left_el right_el )
        with
        | Less, Less, _ -> t
        | Greater, _, Less ->
            TwoBranch (balance, left_el, fix (__replace_top left el), right)
        | _, Greater, Greater | _, Greater, Equal ->
            TwoBranch (balance, right_el, left, fix (__replace_top right el))
        | _ ->
            failwith
              "(Less, Greater, Less) is not logically possible. Check for an \
               error somewhere.")

  let extract_tree (q : queue) : tree =
    match q with Empty -> raise QueueEmpty | Tree t -> t
  ...
  (*
      If leaf, return elt and empty
      If OneBranch, return child as elt and parent as leaf
      If TwoBranch 
        If balance is odd, meaning the left side has one more node than the 
        right side:
          The value we want to return is somewhere below on the left side
          When we remove it, the tree will become even
          So, change the current node to even and head to the left child. 
          However, if the left child is a leaf, we need to return a OneBranch 
            for that instead

        If Balance is even:
          The value we want is somewhere on the right side 
          When we remove it, the tree will become odd 
          So, change the current node to odd and head to the right child
          If the right child is a leaf, we need to return that and make its 
          parent a Onebranch
  ...
  let rec get_last (t : tree) : elt * queue =
    match t with
    | Leaf el -> (el, Empty)
    | OneBranch (parent, child) -> (child, Tree (Leaf parent))
    | TwoBranch (balance, el, left, right) -> (
        match balance with
        | Odd -> (
            let res, q = get_last left in
            match q with
            | Empty -> (res, Tree (OneBranch (el, get_top right)))
            | _ -> (res, Tree (TwoBranch (Even, el, extract_tree q, right))))
        | Even -> (
            let res, q = get_last right in
            match q with
            | Empty -> (res, Tree (OneBranch (el, get_top left)))
            | _ -> (res, Tree (TwoBranch (Odd, el, left, extract_tree q)))))
  ...
  let take (q : queue) : elt * queue =
    ...
    | TwoBranch (Odd, el, left, right) -> (
        let last, new_queue = get_last left in
        match new_queue with
        | Empty -> (el, Empty)
        | Tree new_left ->
            (el, Tree (fix (TwoBranch (Even, last, new_left, right)))))

  let test_get_top () : unit =
    let b = Elt.generate () in
    let c = b in
    let a = Elt.generate_lt b in
    let d = Elt.generate_gt b in
    (* Queue for normal testing purposes *)
    let queue = empty |> add b |> add c |> add a |> add d in
    passert ~suite:"get_top"
      (get_top (extract_tree queue) = a)
      "multiple elements"

  let test_fix () : unit =
    let test = passert ~suite:"fix" in
    let b = Elt.generate () in
    let c = b in
    let a = Elt.generate_lt b in
    let d = Elt.generate_gt b in
    (* Queue for normal testing purposes *)
    let queue = empty |> add b |> add c |> add a |> add d in
    (* Tree that intentionally breaks the invariant and needs to be fixed  *)
    let _a = Elt.generate_lt a in
    let e = Elt.generate_gt d in
    let viol_tree : tree = __replace_top (extract_tree (add _a queue)) e in
    let fixed = fix viol_tree in
    test (get_top viol_tree = e) "gen unfixed tree";
    test (get_top fixed = a) "fixed tree head"

  let test_get_last () : unit =
    let test = passert ~suite:"get_last" in
    let b = Elt.generate () in
    let c = b in
    let a = Elt.generate_lt b in
    let d = Elt.generate_gt b in
    let _a = Elt.generate_lt a in
    let e = Elt.generate_gt d in
    let q0 = empty |> add b |> add c |> add a |> add d |> add _a |> add e in
    let el1, q1 = get_last (extract_tree q0) in
    let el2, q2 = get_last (extract_tree q1) in
    let el3, q3 = get_last (extract_tree q2) in
    let el4, q4 = get_last (extract_tree q3) in
    let el5, q5 = get_last (extract_tree q4) in
    let el6, q6 = get_last (extract_tree q5) in
    test (get_top (extract_tree q0) = _a) "gen tree";
    test (el1 = e) "pop 1";
    test (el2 = c) "pop 2";
    test (el3 = d) "pop 3";
    test (el4 = a) "pop 4";
  ...
  let test_take () : unit =
    ...
    let e = Elt.generate_gt d in
    let q0 = empty |> add b |> add c |> add a |> add d |> add _a |> add e in
    let el1, q1 = take q0 in
    let el2, q2 = take q1 in
    let el3, q3 = take q2 in
    let el4, q4 = take q3 in
    let el5, q5 = take q4 in
    let el6, q6 = take q5 in
    test (get_top (extract_tree q0) = _a) "gen tree";
    test (el1 = _a) "take 1";
    test (el2 = a) "take 2";
    test (el3 = b) "take 3";
