...
(* half pitch -- Returns a note of the given `pitch` that is a half of a
   measure long *)
let half (pt : pitch) : obj = Note (pt, 0.5, cDEFAULT_VOLUME)

(* quarter pitch -- Returns a note of the given `pitch` that is a
   quarter of a measure long *)
let quarter (pt : pitch) : obj = Note (pt, 0.25, cDEFAULT_VOLUME)

(* eighth pitch -- Returns a note of the given `pitch` that is an eighth
   of a measure long *)
let eighth (pt : pitch) : obj = Note (pt, 0.125, cDEFAULT_VOLUME)
...
let int_to_p : int -> p =
  ...
  fun n -> if n < 0 || n > 11 then raise InvalidPitch else List.nth pitches n

(* time_of_event e -- Given an event `e`, returns at what relative
   time it occurs *)
let time_of_event (event : event) : float =
  match event with Tone (time, _, _) | Stop (time, _) -> time
...
let shift_start (offset : float) (str : event NLS.stream) : event NLS.stream =
...
let cHEADER =
  "4D546864000000060001000100" ^ int_to_hex cTICKS_PER_Q ^ "4D54726B"
...
let rec stream_to_hex (n : int) (str : event NLS.stream) : string =
  if n = 0 then ""
    ...
    | NLS.Cons (Stop (t, pitch), tl) ->
        time_to_hex t ^ pitch_to_hex pitch ^ "00" ^ stream_to_hex (n - 1) tl
...
let list_to_stream (lst : obj list) : event NLS.stream =
  let rec list_to_stream_aux (original : obj list) (remaining : obj list)
      (delay : float) : event NLS.stream =
    match remaining with
    | [] -> list_to_stream_aux original original delay
    | Note (pitch, duration, volume) :: tl ->
        lazy
          (NLS.Cons
             ( Tone (delay, pitch, volume),
               lazy
                 (NLS.Cons
                    (Stop (duration, pitch), list_to_stream_aux original tl 0.))
             ))
    | Rest duration :: tl -> list_to_stream_aux original tl (delay +. duration)
  in
  list_to_stream_aux lst lst 0.

(*......................................................................
  Problem 9. Write a function `pair` that merges two event streams. Events
  that happen earlier in time should appear earlier in the merged
  stream. See below for some examples.
  ......................................................................*)

let rec pair (a : event NLS.stream) (b : event NLS.stream) : event NLS.stream =
  let head_a = NLS.head a in
  let head_b = NLS.head b in
  if time_of_event head_a < time_of_event head_b then
    lazy
      (NLS.Cons
         ( head_a,
           pair (NLS.tail a) (shift_start (-1. *. time_of_event head_a) b) ))
  else
    lazy
      (NLS.Cons
         ( head_b,
           pair (shift_start (-1. *. time_of_event head_b) a) (NLS.tail b) ))

(*......................................................................
  Problem 10. Write a function `transpose` that takes an event stream and
...
let transpose (str : event NLS.stream) (half_steps : int) : event NLS.stream =
  let handler (ev : event) =
    match ev with
    | Tone (time, pitch, vol) ->
        Tone (time, transpose_pitch pitch half_steps, vol)
    | Stop (time, pitch) -> Stop (time, transpose_pitch pitch half_steps)
  in
  NLS.smap handler str
...
let melody1 = list_to_stream [ quarter (C, 3); quarter_rest; half (E, 3) ]
...
let melody2 = shift_start 0.25 (transpose melody1 7)
...
(*......................................................................
  Problem 11: Then with just three lists provided after this comment and
...
let bass =
  list_to_stream
    (List.map quarter
       [ (D, 3); (A, 2); (B, 2); (Gb, 2); (G, 2); (D, 2); (G, 2); (A, 2) ])
...
(*......................................................................
  Problem 12: Challenge problem. There's lots of opportunity for
