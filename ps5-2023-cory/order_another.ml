...
module StringLengthCompare : COMPARABLE with type t = string = struct
  type t = string
  ...
  let generate () =
    string_of_int (Random.int (int_of_float ((2. ** 30.) -. 2.)))

  let generate_gt s = s ^ " ++"

  let generate_lt s =
    try String.sub s 0 (String.length s - 1)
    with (* In the case that there's nothing to shorten from *)
    | _ -> ""
