...
let parse (s : string) : expression =
    ...
    | t :: ts ->
       if t = delim then 
		 ...
		 if d then (Binop (b, eq, eq2), l', true)
            ...
            | _ -> 
               raise 
		 (ParseError ("Expecting Binop, but found: "
			      ^ token_to_string t))
		 
  and parse_delimited_expression (l : token list)
				 (delim : token)
				 (current_prec : int)
                               : expression * token list * bool =
    match l with
  ...
  and parse_unop (tokens:token list)
		 (u : unop)
