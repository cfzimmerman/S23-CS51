...
let rec smap3 (f : 'a -> 'b -> 'c -> 'd) (s1 : 'a stream) (s2 : 'b stream)
    (s3 : 'c stream) : 'd stream =
  lazy
    (Cons
       (f (head s1) (head s2) (head s3), smap3 f (tail s1) (tail s2) (tail s3)))

let aitkin_f (back2 : float) (back1 : float) (current : float) : float =
  current
  -. (Float.pow (current -. back1) 2. /. (current -. (2. *. back1) +. back2))

let aitken (s : float stream) : float stream =
  smap3 aitkin_f s (tail s) (tail (tail s))
...
let rec print_indents (num : int) : unit =
  if num > 0 then (
    Printf.printf "  ";
    print_indents (num - 1))
...
let rec tmap2 (f : 'a -> 'b -> 'c) (t1 : 'a tree) (t2 : 'b tree) : 'c tree =
  lazy
    (Node
       (f (node t1) (node t2), List.map2 (tmap2 f) (children t1) (children t2)))
...
let tree_nats : int tree =
  let rec get_children (value : int) : int tree list =
    let new_left = (value * 2) + 1 in
    let new_right = new_left + 1 in
    [
      lazy (Node (new_left, get_children new_left));
      lazy (Node (new_right, get_children new_right));
    ]
  in

  lazy (Node (0, get_children 0))
