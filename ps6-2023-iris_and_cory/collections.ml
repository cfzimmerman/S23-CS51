...
module MakeStackList (Element : sig
  type t
end) : COLLECTION with type elt = Element.t = struct
  exception Empty
  ...
  let is_empty (d : collection) : bool = d = empty
  let length (d : collection) : int = List.length d
  let add (e : elt) (d : collection) : collection = e :: d

  let take (d : collection) : elt * collection =
    match d with hd :: tl -> (hd, tl) | _ -> raise Empty
end
...
module MakeQueueList (Element : sig
  type t
end) : COLLECTION with type elt = Element.t = struct
  exception Empty
  ...
  let length (d : collection) : int = List.length d
  let is_empty (d : collection) : bool = d = empty
  let add (e : elt) (d : collection) : collection = d @ [ e ]
...
(* 🚨 Note the added test file: extra_tests.ml *)

module MakeQueueStack (Element : sig
  type t
end) : COLLECTION with type elt = Element.t = struct
  exception Empty

  type elt = Element.t
  type collection = Element.t list * Element.t list

  let empty : Element.t list * Element.t list = ([], [])
  let length (lead, reverse) : int = List.length lead + List.length reverse
  let is_empty ((lead, reverse) : collection) : bool = lead = [] && reverse = []

  let add (el : Element.t) ((lead, reverse) : collection) : collection =
    (lead, el :: reverse)

  let rec take ((lead, reverse) : collection) : Element.t * collection =
    match (lead, reverse) with
    | [], [] -> raise Empty
    | hd :: tl, _ -> (hd, (tl, reverse))
    | [], _ -> take (List.rev reverse, [])
end
...
let reflection () : string =
  "Building a queue from two stacks was a ton of fun, as was the puzzle \
   solver. Although, working on `experiments`, it took surprisingly little \
   time to run into huge time barries to running the solver (like unable to \
   solve 4x4 tile game in under 5 mins). Is this just the nature building a \
   puzzle solver, or are we implementing super inefficient, brute-force \
   approaches where there's actually a more elegannt algorithmic solution \
   possible? If the latter, I do hope we revisit how to  \n\
   actually do this better at some point in this class."
