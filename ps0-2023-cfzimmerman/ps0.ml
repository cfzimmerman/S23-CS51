...
let name : string * string = ("Cory", "Zimmerman")
...
(* 5. Replace the string below with a message to post to the problem
   set website. *)
let exciting : string =
  "I'm looking forward to practicing solid recursive algorithm design."
...
(* 7. Set this variable to the key you got from <http://ps0.cs51.io/>
   ...
   there. *)
let key : string = "7224e180-bc71-48aa-abef-6c473cd72c9a"
...
let post_survey s =
  ...
  http_post url mbody |> fun x ->
  from_string x |> Util.member "status" |> fun x ->
  to_string x |> print "%s\n";
  print_survey s
;;

let s = convert_survey () in
s |> if post then post_survey else print_survey
